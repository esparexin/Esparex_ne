...
execSync('npx --yes knip --reporter json > .current-knip.json');
execSync('npx --yes jscpd . --min-lines 5 --min-tokens 50 --reporters json --output .current-jscpd');
const current = {
...
const baseUnused = baseline.knip.files.length + baseline.knip.exports.length;
const currUnused = current.knip.files.length + current.knip.exports.length;
